...
- **Встроенный Web UI:** Интерактивный дашборд с умным авто-обновлением (защищен Basic Auth).
  - Управление ключами: добавление пачкой через текстовую область и удаление в один клик без перезапуска сервера и без использования текстовых файлов.
...
Вставьте логин/пароль (по умолчанию `admin` / `admin`). На странице вы увидите:
1. **Управление ключами:** форма добавления новых API ключей списком (каждый с новой строки) и кнопки удаления для каждого ключа.
2. **Общую сводку:** Всего запросов, запросы сегодня, статус ключей (активные/лимит/невалидные).
3. **🏆 Топ Free моделей:** актуальный рейтинг с Shir-Man API, включая алиасы `top1/2/3`.
4. **📊 Статистику моделей:** какие модели запрашивали чаще всего, среднее время ответа (latency) и объём пропущенных токенов.
5. **🔑 Детализацию по ключам:** список всех аккаунтов, их текущее состояние, сколько запросов совершено сегодня, остаток лимита в OpenRouter, процент ошибок и таймеры cooldown.
